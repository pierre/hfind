...
You can simply run ./hfind from the exploded tarball directory:
    ...
    000"
    i[pierre@mouraf ~/downloads]$ ./hfind /user/pierre -type f -name "file*.dat" -o -type d -name a
...
hfind follows[1] the POSIX specification documented here:

    http://www.opengroup.org/onlinepubs/009695399/utilities/find.html

1. As close as possible. As of 2010, some options cannot be implemented (e.g. symbolic links).
-depth/-d follows the BSD implementation. See TODO below for work in progress.
...
* Implement -prune (has no effect today)
